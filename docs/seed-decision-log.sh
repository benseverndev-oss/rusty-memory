...
echo; echo -n "policy "
d "Keep most_recent as the shipped default" "opt an attribute into valid_interval where its history matters, as the template already demonstrates"   --because "the policy reaches exactly one command -- about -- because the decision reads build their own timeline and assert_resolved is never called from a host path, and the live store holds no remember-written facts at all, so switching would change behaviour no deployment can observe. Since #45 about --valid-at refuses under most_recent and names the config line that would fix it, which is honest rather than wrong. Cost was measured and did not decide it: 140ns between the two at the depth this store is at. Revisit when remember is exposed over MCP and facts start being written, which makes about a live path"
d "Record a decision when it is made, not when the work settles" "write it at the moment of deciding, including decisions expected to be revised"   --because "a peer session had a real supersession inside a few hours -- decided, shipped, found wanting, remade -- and recorded one record holding the final state because it wrote up at end of day, so the intermediate belief never reached the store. This store shows the same signature: one rejected decision among 219, its status slot at depth 1. Recording after the fact collapses supersessions and makes a store look like nothing is ever revised, which is a habit masquerading as a finding"
echo
